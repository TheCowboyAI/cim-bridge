...
async fn main() -> Result<()> {
    ...
    client
        .publish(
            ...
            list_models_command.to_string().into(),
        )
        .await?;

    // Wait for response
    if let Some(msg) =
        tokio::time::timeout(std::time::Duration::from_secs(5), event_sub.next()).await?
    {
        let response_text = String::from_utf8(msg.payload.to_vec())?;
        println!("Models Response: {response_text}\n");
    } else {
    ...
    tokio::select! {
        Ok(Some(msg)) = tokio::time::timeout(
            ...
            let response_text = String::from_utf8(msg.payload.to_vec())?;
            println!("Chat Response: {response_text}\n");
        }
        Ok(Some(msg)) = tokio::time::timeout(
            std::time::Duration::from_secs(60),
            failure_sub.next()
            ...
            let response_text = String::from_utf8(msg.payload.to_vec())?;
            println!("Chat Failed: {response_text}\n");
        }
        else => {
    ...
    let response = client
        .request(
            "bridge.query.active_provider",
            provider_query.to_string().into(),
        )
        .await?;

    let response_text = String::from_utf8(response.payload.to_vec())?;
    println!("Active Provider Response: {response_text}\n");
    ...
    client
        .publish(
            ...
            list_providers_command.to_string().into(),
        )
        .await?;

    // Wait for response
    if let Some(msg) =
        tokio::time::timeout(std::time::Duration::from_secs(5), providers_sub.next()).await?
    {
        let response_text = String::from_utf8(msg.payload.to_vec())?;
        println!("Providers Response: {response_text}\n");
    } else {
